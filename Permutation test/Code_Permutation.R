setwd("")#setting working path
library(tidyverse)

# code for the permutation test

in_file <- "SourceData_Permutation.csv"#setting inpout file

sg <- read_csv(in_file, guess_max=10000) %>%
  #filter(!is.na(group_summary) & !is.na(S_SC_STc)) %>%
...
p_vals <- sg_ran_r_c %>%
  ...
  arrange(p_adj)
write.table(p_vals, file = paste0("Result_Permutaion.csv"), quote = FALSE, sep = "\t")
